...
void Renderer::MouseProcessing(int button)
{
	...
	if (scn->isPicked )
	{
		if (button == 1)
		{
			...
			//assignment3
			if (!std::strcmp(&scn->data().model[0], "sphere")) {
		...
		else
		{
			...
			if (!std::strcmp(&scn->data().model[0], "sphere")) {
				...
				//for (int i = 1; i < scn->data_list.size(); i++) {
					for(int j=scn->selected_data_index; j<scn->data_list.size();j++){
					scn->data(j).RotateInSystem(Eigen::Vector3d(1, 0, 0), yrel / 100.0);
					scn->data(j).RotateInSystem(Eigen::Vector3d(0, 1, 0), xrel / 100.0);
