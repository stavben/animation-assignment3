  ...
  if (ImGui::CollapsingHeader("Mesh", ImGuiTreeNodeFlags_DefaultOpen))
  {
    ...
    if (ImGui::Button("Load##Mesh", ImVec2((w-p)/2.f, 0)))
    {
      ...
      if (viewer->data_list.size() > viewer->parents.size())
      {
          ...
          viewer->selected_data_index = savedIndx+1;
        
          
          //assignemnt3

          viewer->parents[viewer->selected_data_index] = viewer->selected_data_index - 1;
          viewer->linkNumber++;
          viewer->data_list[viewer->selected_data_index].SetCenterOfRotation(center.transpose());
          viewer->data_list[viewer->selected_data_index].MyTranslate(Eigen::Vector3d(0, 0, 1.6), true);
          viewer->data_list[viewer->selected_data_index].setAxis();
