...
void igl::opengl::ViewerData::setAxis() {
        
    Eigen::Vector3d m = V.colwise().maxCoeff();
    Eigen::MatrixXd centerOfAxis(1, 3);
    centerOfAxis << V.colwise().mean()[0], V.colwise().mean()[1], m(2);
    Eigen::MatrixXd axisBounding(6, 3);
    axisBounding << 
        centerOfAxis(0) + 1.6, centerOfAxis(1), centerOfAxis(2),
        centerOfAxis(0) - 1.6, centerOfAxis(1), centerOfAxis(2),
        // y
        centerOfAxis(0), centerOfAxis(1) + 1.6, centerOfAxis(2),
        centerOfAxis(0), centerOfAxis(1) - 1.6, centerOfAxis(2),
        // z
        centerOfAxis(0), centerOfAxis(1), centerOfAxis(2) + 1.6,
        centerOfAxis(0), centerOfAxis(1), centerOfAxis(2) - 1.6;

    this->add_points(centerOfAxis, Eigen::RowVector3d(0, 0, 0));
    this->add_edges(axisBounding.row(0), axisBounding.row(1), Eigen::RowVector3d(255, 0, 1));
    this->add_edges(axisBounding.row(2), axisBounding.row(3), Eigen::RowVector3d(255, 1, 0));
    this->add_edges(axisBounding.row(4), axisBounding.row(5), Eigen::RowVector3d(255, 0, 0));
}
...
/*void igl::opengl::ViewerData::setAxis()
{
    ...
    Eigen::Vector3d M = V.colwise().maxCoeff();
       ...
       // double lenOfCy = M(1) - m(1);
       // std::cout << lenOfCy << std::endl;
    ...
    add_points(axisPoints, Eigen::RowVector3d(1, 0, 0));
