...
{
	Renderer* rndr = (Renderer*)glfwGetWindowUserPointer(window);
	std::cout << rndr->IsPicked() << std::endl;
	std::cout << rndr->GetScene()->data().model << std::endl;
	if (rndr->IsPicked()) {
		
		// assignment3
		if(!std::strcmp(&rndr->GetScene()->data().model[0],"sphere"))
		rndr->GetScene()->data().MyScale(Eigen::Vector3d(1 + y * 0.01, 1 + y * 0.01, 1 + y * 0.01));
		else {
			for(int i =1; i< rndr->GetScene()->data_list.size();i++)
				rndr->GetScene()->data_list[i].MyScale(Eigen::Vector3d(1 + y * 0.01, 1 + y * 0.01, 1 + y * 0.01));
		}
		
		
		//data().MyScale(Eigen::Vector3d(1 + y * 0.01,1 + y * 0.01,1+y*0.01));
...
static void glfw_key_callback(GLFWwindow* window, int key, int scancode, int action, int modifier)
{
	...
	if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS)
		...
		case 't':
		{
			...
			//scn->printTips();
			std::cout << scn->getTip().head(3) << std::endl;
			//scn->printTips();
			break;
		}
		case '[':
		...
		case 'd':
		{
			std::cout << scn->spherePosition().head(3) << std::endl;
			break;
		}
		case 'E':
		case 'e':
		{
			for(int i =0; i<scn->tips.size();i++)
			std::cout << scn->tips[i] << std::endl;
			break;
		}
		case GLFW_KEY_C:
		...
		case GLFW_KEY_UP:
			...
			*/
			...
			//scn->data().RotateInSystem(scn->data().MakeTransd(), Eigen::Vector3d(1, 0, 0), 0.05);
			break;
		case GLFW_KEY_DOWN:
			//rndr->TranslateCamera(Eigen::Vector3f(0, -0.01f,0));
			//scn->data().TranslateInSystem(scn->GetRotation(), Eigen::Vector3d(0,-0.5, 0));
		...
		case GLFW_KEY_LEFT:
			...
			//scn->data().MyRotate(Eigen::Vector3d(0,1,0), 0.05);
			//assignemt3
